...
def verify_corners(matrix, region_type, i, j):
    ...
    top_left_corner = (
        (
            (j - 1 < 0 or matrix[i][j - 1] != region_type) and
            (i - 1 < 0 or matrix[i - 1][j] != region_type)
        ) or
        (
            (j - 1 >= 0 and matrix[i][j - 1] == region_type) and
            (i - 1 >= 0 and matrix[i - 1][j] == region_type) and
            matrix[i - 1][j - 1] != region_type
        )
    )
    top_right_corner = (
        (
            (j + 1 >= m or matrix[i][j + 1] != region_type) and
            (i - 1 < 0 or matrix[i - 1][j] != region_type)
        ) or
        (
            (j + 1 < m and matrix[i][j + 1] == region_type) and
            (i - 1 >= 0 and matrix[i - 1][j] == region_type) and
            matrix[i - 1][j + 1] != region_type
        )
    )
    bottom_left_corner = (
        (
            (j - 1 < 0 or matrix[i][j - 1] != region_type) and
            (i + 1 >= n or matrix[i + 1][j] != region_type)
        ) or
        (
            (j - 1 >= 0 and matrix[i][j - 1] == region_type) and
            (i + 1 < n and matrix[i + 1][j] == region_type) and
            matrix[i + 1][j - 1] != region_type
        )
    )
    bottom_right_corner = (
    ...
    def calc_area_and_perimeter(region_type, i, j):
        ...
        for m1, m2 in moves:
            ...
            if (
                ...
                area, perimeter1, perimeter2 = calc_area_and_perimeter(region_type, new_row, new_col)
                total_area += area
    ...
    for i in range(n):
        for j in range(m):
            if (i, j) not in visited:
                area, perimeter1, perimeter2 = calc_area_and_perimeter(matrix[i][j], i, j)
